...
sub_sig = capp.signature('core.subdomains', debug=True,
                                            args=[domain]).set(queue='core')
sig_list.append(sub_sig)

live_sig = capp.signature('core.livedomain', debug=True,
                                        args=[]).set(queue='core')
sig_list.append(live_sig)

live_sig = capp.signature('core.ports', debug=True,
                                        args=[]).set(queue='core')
sig_list.append(live_sig)

live_sig = capp.signature('core.directories', debug=True,
                                        args=[]).set(queue='core')
sig_list.append(live_sig)
